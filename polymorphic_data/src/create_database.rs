...
/// # Returns
///
/// A `Result` containing a `Vec<Document>` if successful, or a `Box<dyn Error>`
/// if an error occurred during file reading or deserialization.
async fn read_json_file(file_path: &str) -> Result<Vec<Document>, Box<dyn Error>> {
    ...
    // Deserialize the JSON string into a vector of Document structs
    let data: Vec<Document> = serde_json::from_str(&json_string)?;
...
pub(crate) async fn handler_database(
    ...
    let contents: Vec<Document> = match read_json_file(file_path).await {
        Ok(items) => {
